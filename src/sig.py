...
logger.add("../hjw_examples/statics/logs/sig.log", level="INFO", rotation="10MB", encoding="utf-8", enqueue=True, retention="10 days")
...
def trend_reverse_ubi(c: CZSC, fx_dt_limit: int = 5, **kwargs) -> OrderedDict:
    ...
    **信号逻辑：**
    ...
    if history.check_duplicate(symbol, edt, days=30 * 6, signals='一买'):
        ...
        print(bis[-1].raw_bars[-1].cache[cache_key]['dif'] > bis[-1].raw_bars[-1].cache[cache_key]['dea'] > 0)
