...
from src.sig.utils import select_failed_conditions, date_exceed_rawbars
...
def third_buy_bi(c: CZSC, fx_dt_limit: int = 5, **kwargs) -> OrderedDict:
    ...
    **信号逻辑：**
    ...
    zs2_fake_bis = zs2.bis[-1].fake_bis
    print(f"{len(zs2_fake_bis)=}")

    third_buy_conditions = (
        (ubi['direction'] == Direction.Up, "ubi['direction'] == Direction.Up"),
        (len(ubi['fxs']) < 2, "len(ubi['fxs']) < 2"),
        (zs2.bis[0] == Direction.Down, "zs2.bis[0] == Direction.Down"),
        (len(zs2.bis) == 1, "len(zs2.bis) == 1"),
        (3 < len(zs2_fake_bis), "3 < len(zs2_fake_bis)"),
    ...
    if not failed_third_buy_conditions:
        ...
        logger.info(f"{name}{symbol}盘整背驰不成立原因: {failed_third_buy_conditions}")
    return create_single_signal(k1=k1, k2=k2, k3=k3, v1=v1)