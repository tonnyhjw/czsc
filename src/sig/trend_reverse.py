    ...
    if history.check_duplicate(symbol, edt, days=30 * 6, signals='一买', db=db):
        ...
        if not failed_bis_pzbc_conditions:
            ...
            if (
                ...
                v1 = '二买'
                if len(zs1_after_1st_buy.bis) > 2:
                    v1 = '类二买'
                # 插入数据库
                history.insert_buy_point(name, symbol, ts_code, freq, v1, latest_fx.power_str, estimated_profit,
                                         industry, latest_fx.dt,  latest_fx.high, latest_fx.low,
...
def fake_xd_2nd_buy_point(c: CZSC, fx_dt_limit: int = 5, **kwargs) -> OrderedDict:
    """模拟线段二买
    **信号逻辑：**

    1. 三个中枢按方向“下上下”排列
    2. 第一个中枢（向下）的DD是三个中枢的最低点
    3. 第二个中枢（向上）
        1. 第一笔至少要回到第一中枢内部
        2. 第一笔的low就是该中枢的DD
    4. 第三个中枢（向下）
        1. 最后底分型收到MA250支撑，底分型第三元素最终收盘价高于MA250
        2. 排除弱底分型
        3. 至少3笔
    ...
    zs1, zs2, zs3 = zs_seq[-3], zs_seq[-2], zs_seq[-1]
    if len(zs1.bis) < 3 or len(zs2.bis) < 3 or len(zs3.bis) < 3:
        v1 = '中枢笔不足'
        return create_single_signal(k1=k1, k2=k2, k3=k3, v1=v1)

    estimated_profit = (zs2.zg - cur_price) / cur_price

    def _zs_direction(zs_direction: Direction, zs: ZS):
        return zs.sdir == zs_direction and zs.edir == zs_direction

    def _zs1_ending_has_min_dd():
        return zs1.dd == min(zs1.dd, zs2.dd, zs3.dd) == zs1.bis[-1].low
    ...
    if not failed_fake_xd_2nd_buy_conditions:
        ...
        logger.info(f"{name}{symbol}二买Fake不成立原因: {failed_fake_xd_2nd_buy_conditions}")
    return create_single_signal(k1=k1, k2=k2, k3=k3, v1=v1)